...
def makeContactSheet(frameNames, grabTimes, fileInfo, sheetParams, tcParams, infoParams, tempDir, info, verbose):
    ...
    for frame in frameNames:  # removing borders from screen shots
        ...
        if verbose:
            ...
            print("--- Trimmed size width x height: %s x %s px" %
                  (trWidth, trHeight))
        trimmedFileName = "%s/%s.trimmed.png" % (
            tempDir, os.path.splitext(os.path.basename(frame))[0])
    ...
    thumbWidth = (sheetWidth - leftMargin - rightMargin -
                  (sheetColumns - 1) * thumbPadding) / sheetColumns
    thumbHeight = int(thumbWidth / trimmedRatio)

    if info or verbose:
        print("--- Trimmed width x height: %s x %s px" %
              (trimmedWidth, trimmedHeight))
        print("--- Trimmed ratio: %s" % trimmedRatio)
        print("--- Thumbs will be width x height: %s x %s px" %
              (thumbWidth, thumbHeight))

    print("--- Creating thumbs")
    thumbs = makeThumbs(trimmedFrames, tcParams, grabTimes, thumbWidth,
                        thumbHeight, tempDir, verbose)  # create thumbs from screen shots
    # read in all images and resize appropriately
    thumbs = [Image.open(frame).resize((thumbWidth, thumbHeight))
              for frame in thumbs]

    # calculate the size of the output image, based on the photo thumb sizes, margins, and padding
    marginsWidth = leftMargin + rightMargin
    ...
    contactSheet = Image.new('RGB', contactSheetSize,
                             sheetBackground)  # create the new image

    if info or verbose:
        print("--- Contact sheet is width x height: %s x %s px" %
    ...
    for rowNo in range(sheetRows):  # insert thumbs into contact sheet
        for columnNo in range(sheetColumns):
            left = leftMargin + columnNo * \
                (thumbWidth + thumbPadding)  # left X coordinate
            upper = infoHeight + topMargin + rowNo * \
                (thumbHeight + thumbPadding)  # left Y coordinate
            ...
            thumbNo += 1
            if verbose:
                print("--- Inserting thumb #%s at %s, %s, %s, %s. Gives a thumb size of %s x %s px" %
                      (thumbNo, left, upper, right, lower, right - left, lower - upper))
            else:
    ...
    contactSheet = addInfo(contactSheet, infoParams,
                           fileInfo, infoHeight, verbose)
...
def removeBorders(myFile, verbose):
    ...
    if trim:
        # create a new image with the same colour as pixel at 0, 0
        background = Image.new(myFile.mode, myFile.size,
                               myFile.getpixel((0, 0)))
        # the absolute value of the difference between the original image and the new image
        diff = ImageChops.difference(myFile, background)
...
def makeThumbs(trimmedFrames, tcParams, grabTimes, thumbWidth, thumbHeight, tempDir, verbose):
    ...
    for frame in trimmedFrames:
        frameNo += 1
        thumbFileName = "%s/%s.thumb.png" % (tempDir,
                                             os.path.splitext(os.path.basename(frame))[0])
        if verbose:
            print("--- Creating thumb #%s to width x height: %s x %s px" %
                  (frameNo, thumbWidth, thumbHeight))
        else:
            countDown = noFrames - frameNo + 1
            progress = "%s " % countDown
            sys.stdout.write(progress)
        ...
        if verbose:
            ...
            print("--- New size width x height: %s x %s px" %
                  (thWidth, thHeight))

        if timeCode:  # if time code should be added to thumb
            # time when screen shot was taken
            grabTime = str(timedelta(seconds=grabTimes[frameNo - 1]))
            ...
            thumb = printTextOutline(
                thumb, tcX, tcY, grabTime, tcOutlineColour, tcFont, tcSize)  # print text outline
            draw = ImageDraw.Draw(thumb)
            font = ImageFont.truetype(tcFont, tcSize)
            # drawing with the fill colour
            draw.text((tcX, tcY), grabTime, tcColour, font=font)
...
def printTextOutline(image, posX, posY, text, outlineColour, font, fontSize):
    ...
    draw.text((posX + 1, posY), text, outlineColour,
              font=font)  # drawing the outline colour
...
def addInfo(contactSheet, infoParams, fileInfo, infoHeight, verbose):
    ...
    draw.text((posX, posY), "File name: %s.%s" % (
        fileInfo['fileName'], fileInfo['fileExtension']), infoColour, font=font)
    draw.text((posX, posY + infoSize + paddingY), "Duration: %s" %
              fileInfo['generalDuration'], infoColour, font=font)
    draw.text((posX, posY + infoSize * 2 + paddingY * 2),
              "File size: %s" % fileInfo['fileSize'], infoColour, font=font)
    draw.text((posX, posY + infoSize * 3 + paddingY * 3), "Width x height: %s x %s px" %
              (fileInfo['width'], fileInfo['height']), infoColour, font=font)
    return contactSheet
