...
config.read("%s/config.ini" %
            os.path.dirname(os.path.realpath(__file__)))  # read config file

videoTypes = (config.get('video', 'videoTypes')
              ).split(',')  # allowed file types

sheetWidth = int(config.get('contactsheets', 'sheetWidth')
                 )  # paramaters for the contact sheet
...
sheetParams = [sheetWidth, sheetHeight, sheetColumns, sheetRows, leftMargin,
               topMargin, rightMargin, bottomMargin, thumbPadding, sheetBackground, infoHeight]

startOffset = int(config.get('frameGrabbing', 'startOffset')
                  )  # parameters for the frame grabs
...
tempDir = os.path.join(os.path.expanduser("~"), config.get(
    'paths', 'tempDir'))  # temporary dir, used to store frame grabs
            ...
            if not os.access(path, os.W_OK):
                ...
                while True:
                    ...
                    if createDir.lower() == "y" or not createDir:
                        ...
                        onError(7, "Opted not to create directory %s" %
...
def checkFileName(myFile, keepGoing, noRename, outDir, verbose):
    ...
    if fileName != re.sub(allowedCharsString, '_', fileName):
        if noRename:
            print(
                "*** Filename does not comply to our rules,\n    but renaming has been disabled")
        else:
            print(
                "*** %s does not comply to our file naming standard\n    Will rename..." % fileName)
            print("--- New file name will be: %s" %
                  re.sub(allowedCharsString, '_', fileName))
            ...
            if os.path.isfile(os.path.join(path,
                ...
                while True:
                    ...
                    print(("--- Renaming it to %s" %
                           os.path.join(path, "%s_%s%s" %
                                        (newName, number, suffix))
                           ))
                    if os.path.isfile(os.path.join(path,
                        ...
                        myFile = os.path.join(
                            path, "%s_%s%s" % (newName, number, suffix))
                        break
            else:
                os.rename((os.path.join(path, "%s%s" % (fileName, suffix))),
                          (os.path.join(path, "%s%s" % (newName, suffix)))
...
def confirm(message, verbose):
    while True:
        answer = input('Are you sure you want to %s? (Y/n) ' % message)
